classdef LUMVE < handle
    ...
    methods
        ...
        function run(obj, x0, P0)
            ...
            obj.x(:,1) = x0;
            dX         = zeros(size(x0));   % state deviation
            obj.P0     = P0;
            ...
            for i=1:obj.iter    % iterate specified # of times
                ...
                for k=2:obj.s       % iterate through simulation times
                    ...
                    if ismember(tk, obj.t_meas)     % step with measurement
                        ...
                        F = expm(A * tk);               % get STM
                        H = obj.dhdx(tk, x_) * F;       % measurement matrix
                ...
                if i ~= 1 || nargin == 3
                    HTWH = HTWH + inv(obj.P0);
                    HTWy = HTWy + obj.P0 \ dX;
                end
                obj.P0 = inv(HTWH);
                dX_hat = obj.P0 * HTWy;             % state deviation
                obj.x(:,1) = obj.x(:,1) + dX_hat;   % get final estimate
            end
